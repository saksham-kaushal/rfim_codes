...
int main(void)
{
	...
	for(i=0;i<ITERS;i++)
	{
		for(delta=delta_beg;delta<=delta_end;delta+=delta_inc)
		{
			...
			string phiFile="./Phi_new-"+to_string(i)+"-"+to_string(num)+".csv";
			string siFile="./Si_new-"+to_string(i)+"-"+to_string(num)+".csv";
			double tot_en=0.0;
...
//			cout<<"total energy and magnetization per site for delta = "<<delta<<" are "<<tot_en<<" and "<<mag_per_site<<endl<<endl;
			cout<<fixed_float(delta)<<"\t"<<fixed_float(tot_en)<<"\t"<<fixed_float(mag_per_site)<<endl;
		}
