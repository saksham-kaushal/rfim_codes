...
void core(vector<float>& Bmat, double& maxflow, double& energy, double& mag_ps, vector<int>& visited, string phi_str, string si_str, double delta, int iter, double omega) 
{
...
//			savedata(CapacityMat,iter,delta,"Cap");
			savedata(Bmat,iter,delta,phi_str,omega);
			savedata(visited,iter,delta,si_str,omega);
...
int main(void)
{
	...
	double delta, maxflow1, energy1, mag_ps1, maxflow2, energy2, mag_ps2, omega=0.0;
	int iter=0;
	for(iter=0;iter<ITERS;iter++)
	{
		for (delta=delta_beg ; delta<=delta_end; delta+=delta_inc)
		{
			...
			core(Bmat1,maxflow1,energy1,mag_ps1,visited1,"Phi","Si",delta,iter,omega);
			for (omega=omega_beg; omega<omega_end; omega+=omega_inc)
			{
				...
				for (int i=0;i<N;i++)
				{
					Bmat2[i]= Bmat1[i]-omega*(2*visited1[i+1]-1);
				}
				core(Bmat2,maxflow2,energy2,mag_ps2,visited2,"Phi_new","Si_new",delta,iter,omega);
				
				double q = get_q(visited1,visited2);		cout<<fixed_float(delta)<<"\t"<<fixed_float(omega)<<"\t"<<fixed_float(energy1)<<"\t"<<fixed_float(mag_ps1)<<"\t"<<fixed_float(energy2)<<"\t"<<fixed_float(mag_ps2)<<"\t"<<fixed_float(q)<<endl;
			}
		}
	}
	Bmat1.clear();
